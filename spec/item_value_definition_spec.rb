...
describe AMEE::Admin::ItemValueDefinitionList, "with an authenticated connection" do
  ...
  it "should fail gracefully with incorrect XML data" do
    ...
    connection.should_receive(:v3_get).with("/#{AMEE_API_VERSION}/definitions/BD88D30D1214/values;full", {:resultStart=>0, :resultLimit=>10}).and_return(xml)
    connection.should_receive(:expire).with("/#{AMEE_API_VERSION}/definitions/BD88D30D1214/values;full").once
  ...
  it "should fail gracefully with incorrect JSON data" do
    ...
    connection.should_receive(:retries).and_return(0).once
    json = '{}'
    connection.should_receive(:v3_get).with("/#{AMEE_API_VERSION}/definitions/BD88D30D1214/values;full", {:resultStart=>0, :resultLimit=>10}).and_return(json)
    connection.should_receive(:expire).with("/#{AMEE_API_VERSION}/definitions/BD88D30D1214/values;full").once
    lambda{AMEE::Admin::ItemValueDefinitionList.new(connection, "BD88D30D1214")}.should raise_error(AMEE::BadData)
  end
  ...
  it "should fail gracefully on other errors" do
    connection = flexmock "connection"
    connection.should_receive(:retries).and_return(0).once
    connection.should_receive(:v3_get).with("/#{AMEE_API_VERSION}/definitions/BD88D30D1214/values;full", {:resultStart=>0, :resultLimit=>10}).and_raise(Timeout::Error)
